...
reset:
  ...
  lda #%00010000  ;load a reg with 10
...
VIA0:
  bbs6 IFR0Timer1 ;branch if IRQ (bit 6) VIA0 is set
  bbs5 IFR0Timer2 ;branch if IRQ (bit 5) VIA0 is set
  bbs4 IFR0CB1    ;branch if IRQ (bit 4) VIA0 is set
  bbs3 IFR0CB2    ;branch if IRQ (bit 3) VIA0 is set
  bbs2 IFR0SR     ;branch if IRQ (bit 2) VIA0 is set
  bbs1 IFR0CA1    ;branch if IRQ (bit 1) VIA0 is set
  bbs0 IFR0CA2    ;branch if IRQ (bit 0) VIA0 is set
  jmp irq1        ;jump to irq1

VIA1:
  bbs6 IFR1Timer1 ;branch if IRQ (bit 6) VIA1 is set
  bbs5 IFR1Timer2 ;branch if IRQ (bit 5) VIA1 is set
  bbs4 IFR1CB1    ;branch if IRQ (bit 4) VIA1 is set
  bbs3 IFR1CB2    ;branch if IRQ (bit 3) VIA1 is set
  bbs2 IFR1SR     ;branch if IRQ (bit 2) VIA1 is set
  bbs1 IFR1CA1    ;branch if IRQ (bit 1) VIA1 is set
  bbs0 IFR1CA2    ;branch if IRQ (bit 0) VIA1 is set
  jmp irq1        ;jump to irq1
